...
from client_service.utils.constants import CLIENT_ROLE
...
class AuthClient:
    ...
    def is_authorized(self, endpoint='/api/auth/verify', token=''):
        ...
        res = self._make_request(endpoint=endpoint, params={'role': CLIENT_ROLE}, additional_headers=headers)
        if res.status_code == 200:
            return True
        return False

    def get_user(self, endpoint='/api/user', token=''):
        headers = {
            'Authorization': 'Bearer ' + token,
        }
